...
import { exec as youtubeDl } from "youtube-dl-exec";
...
export const videoSummaryTask = task({
  ...
  run: async (payload: VideoSummaryInput) => {
    ...
    // Step 2: Get transcript using youtube-dl-exec
    ...
    // Step 3: Download video using youtube-dl-exec
    const videoPath = await downloadVideo(videoId);

    // Step 4: Extract frames using @ffmpeg/ffmpeg
    const frames = await extractFrames(videoPath, videoId);
    console.log(`Extracted ${frames.length} frames`);

    // Step 5: Analyze frames with Gemini Vision
    ...
    // Step 6: Summarize transcript with Gemini
...
async function getTranscript(videoId: string): Promise<string> {
  try {
    // Use youtube-dl-exec to get subtitles
...
async function extractFrames(
  ...
  try {
    ...
    const frameFiles = files.filter((f: string) => f.startsWith("frame-"));

    frameFiles.forEach((file: string, index: number) => {
      const data = ffmpeg.readFile(file);
      fs.writeFileSync(path.join(framesDir, file), Buffer.from(data));
    });

    await ffmpeg.delete();

    const frames = frameFiles.map((file: string, index: number) => ({
